...
//
	...
	$sqlQuery="Select `central_govt_scheme`, `state_govt_scheme`, `scheme_e`, `scheme_c`, `scheme_g`, `nav_date`, `nps_lite`, `corporate_cg`, `scheme_t_ii_e`, `scheme_t_ii_c`, `scheme_t_ii_g`,`date_insert`  From  `wp_participants_database` Where  1 order by id desc LIMIT 0,1";

	$arrResult = selectNAV($sqlQuery);


//	$arrResult = $wpdb->get_results($sqlQuery);

//	$arrResult = selectAll('wp_nav'," 1 order by id desc LIMIT 0,1");
	...
	if ( ! empty($arrResult) )
	{
		...
		{
			$data['date'] = Date("d-m-Y",$data['date_insert']);

		    $marqueeText = "Central Govt. Scheme - ".$data['central_govt_scheme'].' on '.$data['date']." || ";
		    $marqueeText .= "State Govt. Scheme - ".$data['state_govt_scheme'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme E - ".$data['scheme_e'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme C - ".$data['scheme_c'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme G - ".$data['scheme_g'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme T II E - ".$data['scheme_t_ii_e'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme T II C - ".$data['scheme_t_ii_c'].' on '.$data['date']." || ";
		    $marqueeText .= "Scheme T II G - ".$data['scheme_t_ii_g'].' on '.$data['date']." || ";
		    $marqueeText .= "NPS Lite - ".$data['nps_lite'].' on '.$data['date']." || ";
		    $marqueeText .= "Corporate CG - ".$data['corporate_cg'].' on '.$data['date'];
		}
	}
	...
	$hsa_direction = get_option('hsa_direction');
	$hsa_style = get_option('hsa_style');
	$what_marquee = $what_marquee . "<div style='padding:3px;width: 1000px;'>";
	$what_marquee = $what_marquee . "<marquee style='$hsa_style' scrollamount='$hsa_scrollamount' scrolldelay='$hsa_scrolldelay' direction='$hsa_direction' onmouseover='this.stop()' onmouseout='this.start()'>";
	$what_marquee = $what_marquee . $marqueeText;
	$what_marquee = $what_marquee . "</marquee>";
	$what_marquee = $what_marquee . "</div>";
	return $what_marquee;
}

function selectNAV($sql)
{
